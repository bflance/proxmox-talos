    ...
    for i in $(seq 1 $vm_count); do
        ...
        if qm status $vm_id &>/dev/null; then
            ...
            qm create $vm_id \
                ...
                --efidisk0 $DISK_STORAGE:0,format=raw,efitype=4m,pre-enrolled-keys=0 \
...
EOF
        ...
        echo -e "${BLUE}[$(date +%Y-%m-%d.%H:%M:%S)]${NC} Starting VM $vm_name..."
        qm start $vm_id
    done
...
bootstrap_talos_cluster(){
    echo -e "${BLUE}[$(date +%Y-%m-%d.%H:%M:%S)]${NC} Talos Config Folder: $TALOSCONFIG"
    ...
    until $(talosctl bootstrap -n ${NODE_IPS[0]} 2>/dev/null);do 
      echo -e "${BLUE}[$(date +%Y-%m-%d.%H:%M:%S)]${NC} Trying to bootstrap talos cluster...";
      sleep 5;
    done
    ...
    until $(talosctl kubeconfig . -n ${NODE_IPS[0]} &>/dev/null);do
      echo -e "${BLUE}[$(date +%Y-%m-%d.%H:%M:%S)]${NC} Trying to generate kubeconfig...";
      sleep 2;
    done
